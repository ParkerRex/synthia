...
| A. Panic and diagnostics | [Observed] Sylenth1 has MIDI/all-notes behavior and CPU guidance. | [Observed] sylenth-ai has panic, active voices, MIDI count, invalid sample count, peak, sample rate, block size, architecture diagnostics. | [Inferred] Missing patch cost/CPU estimate and current output meter graphics. | [Inferred] Upgrade. | [Inferred] Header always shows panic, output meter, active voices, invalid sample warning, and patch cost. | [Observed] Panic clears voices and FX; diagnostics use atomics. | [Inferred] Diagnostics preferences should be local state, not preset-critical sound state. | [Observed] Existing panic-clear tests; add output meter smoke and patch-cost deterministic checks. | P0 |
...
| Feature area | Sylenth1 behavior / feature | Current sylenth-ai behavior | Gap | Product decision: Keep / Add / Upgrade / Defer / Avoid | UI/UX requirement | DSP / engine requirement | Preset / state requirement | Validation proof required | Priority |
|---|---|---|---|---|---|---|---|---|---|
| B. Preset selector and previous/next | [Observed] Sylenth1 uses program arrows and a program popup over sub-banks. | [Observed] sylenth-ai has preset combo, Load button, prev/next, and browser-facing source/bank/category/favorite summaries. | [Observed] No polished browser drawer yet. | [Inferred] Upgrade. | [Inferred] Header has preset name, prev/next, save icon/button, dirty indicator, and browser opener. | [Observed] Browser scan/filter/favorite state is outside the audio thread. | [Observed] Current preset paths, display names, source, bank, category, tags, and favorite keys are available. | [Observed] Preset browser metadata/filter/favorite tests plus preset roundtrip. | P0 |
