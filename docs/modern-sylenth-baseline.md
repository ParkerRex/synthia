...
- `[Observed]` The parameter registry now has 332 parameters after adding arp, 16-step, and chord state.
...
| Feature area | Sylenth1 behavior / feature | Current sylenth-ai behavior | Gap | Product decision: Keep / Add / Upgrade / Defer / Avoid | UI/UX requirement | DSP / engine requirement | Preset / state requirement | Validation proof required | Priority |
|---|---|---|---|---|---|---|---|---|---|
| I. Arp modes and timing | [Observed] Sylenth arpeggiator includes Up, Down, Up/Down variants, Random, Ordered, Step Seq, Step Chord, Time, Gate, Wrap, and Octave. | [Observed] sylenth-ai now has `arp.*` APVTS state, fixed 16-step state, chord voice state, and a no-allocation `Arpeggiator` feeding generated notes into the existing voice allocator. | [Observed] Missing richer Random/Ordered variants, full Step Seq/Step Chord UI, and Ableton host validation against the arp path. | [Inferred] Extend modes after v1 timing proves stable; do not block browser/FX roadmap on Random variants. | [Inferred] Main or lower dock has Arp enable, mode, rate, gate, octave, step length, and hold; current UI only exposes top-level arp/chord controls. | [Observed] Arp is sample-scheduled from `tempoBpm`, supports gate, tie, hold, octave wrapping, and panic clearing. | [Observed] Arp, step, and chord fields serialize as ordinary parameters. | [Observed] Direct DSP tests cover order, gate, octave, step pitch/velocity, tie/hold, chord expansion, and panic. | P0 |
