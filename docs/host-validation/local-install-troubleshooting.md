...
```bash
/opt/homebrew/bin/cmake -S . -B build-release -DCMAKE_BUILD_TYPE=Release -DSYNTH_ENABLE_TESTS=ON
...
- AU: `~/Library/Audio/Plug-Ins/Components/Synth.component`
- VST3: `~/Library/Audio/Plug-Ins/VST3/Synth.vst3`
...
```bash
SYNTH_SKIP_ADHOC_SIGN=1 scripts/install-local-plugins.sh build-release
```
...
Remove only Synth's per-user AU and VST3 bundles:
...
4. Rescan plug-ins.
5. Confirm `Synth` appears under both AU and VST3 when installed.

If Synth does not appear after a normal rescan, quit Ableton and inspect Ableton's plugin scan logs before deleting any cache files.
