...
- Standalone UI smoke covers the preset workflow controls at normal and compact sizes: dirty-state pill, Init, Random, Reset, A Store/A Load, B Store/B Load, and the adjacent preset browser/MIDI panels. The A Store manual control smoke confirms A Load becomes actionable after capture.
