...
post_build_recap: Ableton Live 11 exposed `Audio Units > ParkerX > sylenth-ai`, created the current AU, opened a hosted `sylenth-ai/1-sylenth-ai` editor window, ran transport with active voices/meters, and stopped transport cleanly. Explicit hosted editor close/reopen while transport runs remains open.
read_when:
...
It remains a bounded host smoke slice. It does not claim automation, learned CC mapping, preset recreation, modulation exercise, offline-versus-realtime comparison, sample-rate/buffer-size changes, all-notes-off, panic, or explicit hosted editor close/reopen while transport is running.
...
Completed. Ableton created the current AU, opened a hosted editor window, ran MIDI playback with active voices and Live meters, then stopped transport cleanly. The remaining host matrix is narrower but still needs automation, learned CC mapping, current preset recreation/modulation exercise, offline bounce versus realtime comparison, sample-rate/buffer-size changes, all-notes-off, panic, and explicit hosted editor close/reopen while playing.
...
- Explicit hosted editor close/reopen while transport is running.
...
- Docs still leave explicit hosted editor close/reopen while transport is running as open.
